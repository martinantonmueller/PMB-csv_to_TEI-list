...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="root">
        <xsl:element name="TEI" namespace="http://www.tei-c.org/ns/1.0">
            <xsl:element name="teiHeader" namespace="http://www.tei-c.org/ns/1.0">
                <xsl:element name="fileDesc" namespace="http://www.tei-c.org/ns/1.0">
                    ...
                    <xsl:element namespace="http://www.tei-c.org/ns/1.0" name="title"><xsl:text>In den Briefen verwendete PMB-Entitäten</xsl:text></xsl:element></xsl:element>
        ...
        <xsl:for-each select="descendant::rs[@type='person']/tokenize(@ref, ' ')">
            <xsl:sort select="number(.)"/>
            <xsl:if test="string-length(substring-after(., '#')) &gt; 1">
                <xsl:element  namespace="http://www.tei-c.org/ns/1.0" name="person">
                    <xsl:attribute name="n"><xsl:value-of select="normalize-space(substring-after(., '#'))"/></xsl:attribute>
                </xsl:element>
            </xsl:if>
        </xsl:for-each>
        ...
        <xsl:for-each select="descendant::persName[@ref]/@ref">
            ...
            <xsl:if test="string-length(substring-after(., '#')) &gt; 0">
                ...
                <xsl:for-each select="descendant::handNote[@corresp and not(@corresp='Schreibkraft')]/@corresp">
                    <xsl:sort select="number(substring-after(., '#'))"/>
                    <xsl:if test="string-length(substring-after(., '#')) &gt; 0">
                        <xsl:element  namespace="http://www.tei-c.org/ns/1.0" name="person">
                            <xsl:attribute name="n"><xsl:value-of select="normalize-space(substring-after(., '#'))"/></xsl:attribute>
        ...
        <xsl:for-each select="descendant::rs[@type='work']/tokenize(@ref, ' ')">
            ...
            <xsl:if test="string-length(substring-after(., '#')) &gt; 0">
                <xsl:element  namespace="http://www.tei-c.org/ns/1.0" name="item">
                    ...
                    <xsl:for-each select="descendant::biblStruct//title[@ref]/tokenize(@ref, ' ')">
                        <xsl:sort select="number(.)"/>
                        <xsl:if test="string-length(substring-after(., '#')) &gt; 0">
                            <xsl:element  namespace="http://www.tei-c.org/ns/1.0" name="item">
                                <xsl:attribute name="n"><xsl:value-of select="normalize-space(substring-after(., '#'))"/></xsl:attribute>
