...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Dieses XSLT trägt die Institutionstypen in die Liste ein -->
    
    <xsl:param name="ptype-refs" select="document('institution-typen/vocab-institutions.xml')"/>
    ...
    <xsl:template match="tei:gloss[not(@ana)]">
        ...
        <xsl:element name="tei:gloss" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:value-of select="key('ref-lookup', $typ-nr, $ptype-refs)/name"/>
        </xsl:element>
    </xsl:template>
